...
export default function composeData(
	...
	if (!element.hasAttribute) throw new Error(`${JSON.stringify(element)} is missing 'hasAttribute'`);
	const readOnly = element.hasAttribute('readonly');

	if (tag === 'fieldset') {
		if (disabled && !readOnly) for (const nestedField of elements) nestedField.disabled = disabled;
		...
		if (name) {
			...
			if (!disabled) values[name] = _reduce(
				Array.from(elements),
				composeData,
				{ __proto__: null },
			);
