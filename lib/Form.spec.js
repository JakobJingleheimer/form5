...
describe('Form', () => {
	...
	beforeEach(() => {
		initValues = generateFormData(false);
		form = new Form();
	});

	describe('setRef()', () => {
		it('should gather initial values', () => {
			form.setRef({ elements: initValues })
	...
	describe('handleSubmit()', () => {
		it('should prevent default form behaviour & event bubbling', () => {
			...
			form.handleSubmit(event, initValues, () => {});
		...
		it('should supply a delta of initial and current values', () => {
			...
			form.handleSubmit(event, initValues, cb);
